...
/**
 * Class that may be overridden (or used as is) to provide "custom method" data fetcher to the schema building. This class is typically to be extended and used
 * in {@link GQLSchemaBuilder} for buildSchema custom method data fetcher argument
 *
...
public class GQLCustomMethodDataFetcher extends GQLAbstractDataFetcher<Object> {
	...
	public Object get(final DataFetchingEnvironment environment) throws Exception {
		...
		final List<Object> arguments = method.getArgs().stream().map(arg -> getArgumentValue(environment, arg)).collect(Collectors.toList());
		try {
	...
	protected <T> T getArgumentValue(final DataFetchingEnvironment environment, final GQLCustomMethodArg argument) {
		if (GQLExecutionContext.class.isAssignableFrom(GenericsUtils.getTypeClass(argument.getType()))) {
			...
			final Object argumentGraphQLValue = getArgumentValue(queryField, argument.getName(), environment.getArguments());
